-- Move "Second Timer" from a fixed profile-level setting to a per-exam-attempt
-- choice asked on the pre-exam screen. submit_exam_attempt now takes
-- p_is_second_timer directly (from the student's checkbox for THIS attempt)
-- instead of reading profiles.is_second_timer. Also stores the flag on the
-- attempt row itself so recalculation stays accurate per-attempt.

ALTER TABLE public.exam_attempts
  ADD COLUMN IF NOT EXISTS is_second_timer_attempt BOOLEAN NOT NULL DEFAULT false;
...
BEGIN
    ...
    FOR v_attempt IN SELECT id, profile_id, answers, is_second_timer_attempt FROM public.exam_attempts WHERE exam_id = p_exam_id
    LOOP
        v_raw_score := 0;
        v_deduction := 0;
        ...
        IF COALESCE(v_attempt.is_second_timer_attempt, false) AND NOT v_disable_second_timer_deduction THEN
            v_deduction := v_exam_total_marks * 0.03;
        END IF;
...
BEGIN
    ...
    FOR v_attempt IN
        SELECT id, profile_id, answers, is_second_timer_attempt
    ...
    LOOP
        ...
        v_deduction := 0;
        IF COALESCE(v_attempt.is_second_timer_attempt, false) THEN
            v_deduction := v_exam_total_marks * 0.03;
        END IF;
