-- Return topic from get_exam_questions_start so Readymade exams can be
-- filtered/navigated by per-question topic on the student side.
...
RETURNS TABLE(
  ...
  question_index integer, subject text, is_segment_mandatory boolean, topic text
)
