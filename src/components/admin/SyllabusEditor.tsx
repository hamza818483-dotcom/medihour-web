...
import { ChevronDown, ChevronUp, Trash2, Plus, Pencil, Check, X, Layers, Copy } from "lucide-react";
...
export const SyllabusEditor = ({ prefix = "st" }: { prefix?: "st" | "rv" }) => {
  ...
  const { data: subjects, isLoading } = useQuery({
    queryKey: [`admin-${prefix}-subjects`, mode],
    queryFn: async () => {
      const { data, error } = await (supabase.from as any)(`${prefix}_subjects`)
  ...
  const { data: chaptersOfSubject } = useQuery({
    queryKey: [`admin-${prefix}-chapters`, expandedSubject],
    enabled: expandedSubject !== null,
    queryFn: async () => {
      const { data, error } = await (supabase.from as any)(`${prefix}_chapters`)
      ...
      const withCounts = await Promise.all(
        (data || []).map(async (ch: any) => {
          const { count } = await (supabase.from as any)(`${prefix}_topics`)
  ...
  const { data: topicsOfChapter } = useQuery({
    queryKey: [`admin-${prefix}-topics`, expandedChapter],
    enabled: expandedChapter !== null,
    queryFn: async () => {
      const { data, error } = await (supabase.from as any)(`${prefix}_topics`)
  ...
  const refreshSubjects = () => queryClient.invalidateQueries({ queryKey: [`admin-${prefix}-subjects`, mode] });
  const refreshChapters = () => queryClient.invalidateQueries({ queryKey: [`admin-${prefix}-chapters`, expandedSubject] });
  const refreshTopics = () => queryClient.invalidateQueries({ queryKey: [`admin-${prefix}-topics`, expandedChapter] });
  ...
  const addSubject = async () => {
    ...
    try {
      ...
      const { error } = await (supabase.from as any)(`${prefix}_subjects`).insert({
        mode,
  ...
  const addChapter = async () => {
    ...
    setSaving(true);
    try {
      const { data: existing } = await (supabase.from as any)(`${prefix}_chapters`)
        .select("sort_order")
      ...
      const { error } = await (supabase.from as any)(`${prefix}_chapters`).insert({
      ...
      queryClient.invalidateQueries({ queryKey: [`admin-${prefix}-topic-chapter-options`, chapterSubjectId] });
    } catch (e: any) {
      toast({ title: "ব্যর্থ হয়েছে", description: e.message, variant: "destructive" });
    } finally {
      setSaving(false);
  ...
  const addTopics = async () => {
    ...
    setSaving(true);
    try {
      const { data: existing } = await (supabase.from as any)(`${prefix}_topics`)
        .select("sort_order")
      ...
      const { error } = await (supabase.from as any)(`${prefix}_topics`).insert(rows);
      if (error) throw error;
      setTopicNames("");
  ...
  const updateSubject = async (id: number) => {
    ...
    const { error } = await (supabase.from as any)(`${prefix}_subjects`).update({ name: editSubjectName.trim(), short_name: editSubjectName.trim().slice(0, 12) }).eq("id", id);
    if (error) {
  ...
  const updateChapter = async (id: number) => {
    ...
    const { error } = await (supabase.from as any)(`${prefix}_chapters`).update({ name: editChapterName.trim() }).eq("id", id);
    if (error) {
      toast({ title: "ব্যর্থ হয়েছে", description: error.message, variant: "destructive" });
      return;
    }
  ...
  const updateTopic = async (id: number) => {
    ...
    const { error } = await (supabase.from as any)(`${prefix}_topics`).update({ name: editTopicName.trim() }).eq("id", id);
    if (error) {
      toast({ title: "ব্যর্থ হয়েছে", description: error.message, variant: "destructive" });
      return;
    }
  ...
  const updateWeightOf = async (table: "topics" | "chapters" | "subjects", id: number, weight: number | null, refresh: () => void) => {
    const { error } = await (supabase.from as any)(`${prefix}_${table}`).update({ weight }).eq("id", id);
    if (error) {
      toast({ title: "ব্যর্থ হয়েছে", description: error.message, variant: "destructive" });
      return;
    }
  ...
  const updateWeight = (id: number, weight: number | null) => updateWeightOf("topics", id, weight, refreshTopics);
  ...
  const applyTopicsToAllChapters = async (sourceChapId: number, subjId: number) => {
    ...
    setSaving(true);
    try {
      const { data: sourceTopics } = await (supabase.from as any)(`${prefix}_topics`)
        .select("name, weight")
      ...
      const { data: allChapters } = await (supabase.from as any)(`${prefix}_chapters`)
        .select("id")
      ...
      for (const ch of targetChapters) {
        const { data: existingTopics } = await (supabase.from as any)(`${prefix}_topics`)
          .select("name")
          .eq("chapter_id", ch.id);
        const existingNames = new Set((existingTopics || []).map((t: any) => t.name.trim().toLowerCase()));
        const { data: existingSort } = await (supabase.from as any)(`${prefix}_topics`)
          .select("sort_order")
          .eq("chapter_id", ch.id)
          .order("sort_order", { ascending: false })
          .limit(1)
        ...
        if (rowsToInsert.length > 0) {
          const { error } = await (supabase.from as any)(`${prefix}_topics`).insert(rowsToInsert);
          if (error) throw error;
  ...
  const deleteSubject = async (id: number) => {
    ...
    const { error } = await (supabase.from as any)(`${prefix}_subjects`).delete().eq("id", id);
    if (error) {
      toast({ title: "ব্যর্থ হয়েছে", description: error.message, variant: "destructive" });
      return;
    }
  ...
  const deleteChapter = async (id: number) => {
    ...
    const { error } = await (supabase.from as any)(`${prefix}_chapters`).delete().eq("id", id);
    if (error) {
      toast({ title: "ব্যর্থ হয়েছে", description: error.message, variant: "destructive" });
      return;
    }
  ...
  const deleteTopic = async (id: number) => {
    const { error } = await (supabase.from as any)(`${prefix}_topics`).delete().eq("id", id);
    if (error) {
    ...
    try {
      for (let i = 0; i < reordered.length; i++) {
        if (reordered[i].sort_order !== i) {
          const { error } = await (supabase.from as any)(`${prefix}_topics`).update({ sort_order: i }).eq("id", reordered[i].id);
          if (error) throw error;
        }
      }
      refreshTopics();
            ...
            return (
              <div key={s.id} className="border rounded-xl overflow-hidden">
                <div className="w-full flex items-center gap-2 p-3 hover:bg-muted/40">
                  ...
                  {editSubjectId !== s.id && (
                    <PctInput value={s.weight ?? null} eff={subjEff[subjIdx]} onSave={(v) => void updateWeightOf("subjects", s.id, v, refreshSubjects)} />
                  )}
                  {editSubjectId !== s.id && (
                    <Button
                      variant="ghost"
                      ...
                      return (
                        <div key={ch.id} className="border rounded-lg bg-card overflow-hidden">
                          <div className="flex items-center gap-2 p-2.5">
                            ...
                            {editChapterId !== ch.id && (
                              <PctInput value={ch.weight ?? null} eff={chEff[chIdx]} onSave={(v) => void updateWeightOf("chapters", ch.id, v, refreshChapters)} />
                            )}
                            {editChapterId !== ch.id && (
                              <Button
                                variant="ghost"
